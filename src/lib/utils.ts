...
import { type Camera, type Color, type Layer, type Point, type XYWH, type Side, LayerType } from "../types/canvas";
...
export function findLayerAtPoint(
  ...
  for (let i = layerIds.length - 1; i >= 0; i--) {
    ...
    const effectiveTolerance = layer.type === 'path' ? tolerance * 2 : tolerance;
    ...
    if (layer.type === 'path' && layer.points) {
      ...
      if (
        ...
        console.log(`Found path layer at point ${point.x},${point.y}:`, layerId);
        return layerId;
      }
    } else {
      ...
      if (
        ...
        console.log(`Found regular layer at point ${point.x},${point.y}:`, layerId);
        return layerId;
      }
    }
  }
...
export function resizeBounds(
  ...
  if (corner === "bottom" || corner === "bottom-left" || corner === "bottom-right") {
  ...
  if (corner === "right" || corner === "top-right" || corner === "bottom-right") {
