...
function generateCacheKey(messages: MessageData[]): string {
  return messages.map((msg) => `${msg.authorName}:${msg.creationTime}:${msg.body.substring(0, 50)}`).join('|');
}
  ...
  try {
    ...
    if (cachedResult) {
      ...
      if (Date.now() - cachedResult.timestamp < CACHE_TTL) {
        return NextResponse.json({ summary: cachedResult.summary, cached: true });
      } else {
      ...
      const fallbackSummary =
        limitedMessages.length > 5
          ...
          : `### Message Contents\n\n${limitedMessages
              .map((msg: MessageData) => {
                const plainText = extractTextFromRichText(msg.body);
                const formattedDate = format(new Date(msg.creationTime), "MMM d 'at' h:mm a");
