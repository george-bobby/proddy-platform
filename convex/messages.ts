...
export const getMentionedMessages = query({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      if (!currentMember) {
        console.log('getMentionedMessages - No currentMember found for userId:', userId);
        return [];
      }
      ...
      console.log('getMentionedMessages - total messages found:', messages.length);
      ...
      console.log('getMentionedMessages - currentMember._id:', currentMember._id);
      ...
      for (const message of messages) {
              ...
              if (parsedBody.ops) {
                ...
                for (const op of parsedBody.ops) {
                  if (op.insert && typeof op.insert === 'string') {
                    ...
                    if (op.insert.includes(`data-member-id="${currentMember._id}"`)) {
                      hasMention = true;
                      break;
                    }
        ...
        if (hasMention) {
          ...
          const reactionsWithCounts = reactions.reduce((acc, reaction) => {
            const existingReaction = acc.find((r) => r.value === reaction.value);
          ...
          }, [] as Array<Omit<Doc<'reactions'>, 'memberId'> & { count: number; memberIds: Id<'members'>[] }>);
          ...
          if (message.channelId) {
            ...
            if (conversation) {
              const otherMemberId = conversation.memberOneId === message.memberId ? conversation.memberTwoId : conversation.memberOneId;
      ...
      console.log('getMentionedMessages - found mentioned messages:', mentionedMessages.length);
...
export const createTestMentionMessage = mutation({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      console.log("createTestMentionMessage - created message body:", messageBody);
