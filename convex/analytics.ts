...
export const getWorkspaceOverview = query({
  ...
  handler: async (ctx, args) => {
    ...
    const taskStatusData = [
      { label: 'Completed', value: tasks.filter(task => task.status === 'completed').length, color: 'bg-green-500' },
      { label: 'In Progress', value: tasks.filter(task => task.status === 'in_progress').length, color: 'bg-blue-500' },
      { label: 'Not Started', value: tasks.filter(task => task.status === 'not_started').length, color: 'bg-gray-300' },
      { label: 'On Hold', value: tasks.filter(task => task.status === 'on_hold').length, color: 'bg-yellow-500' },
      { label: 'Cancelled', value: tasks.filter(task => task.status === 'cancelled').length, color: 'bg-red-500' },
    ];

    // Get channel activity
    const channelActivityData = await Promise.all(
      channels.map(async (channel) => {
        const channelMessages = messages.filter(message => message.channelId === channel._id);
        return {
          label: channel.name,
...
export const getChannelActivitySummary = query({
  ...
  handler: async (ctx, args) => {
    ...
    const channelActivityPromises = channels.map(async (channel) => {
      ...
      const uniqueUsers = new Set(channelSessions.map(session => session.memberId));
  ...
  handler: async (ctx, args) => {
    ...
    const tasksByDate = dateRange.map(date => {
      ...
      const count = tasks.filter(task =>
        task.createdAt >= day && task.createdAt <= nextDay
      ).length;
